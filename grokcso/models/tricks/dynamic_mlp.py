...
class RecursiveBlock(nn.Module):
    def __init__(self, inplanes, planes, loc_planes, mlp_type='a'):
        super().__init__()
        if mlp_type.lower() == 'a':
            MLP = Dynamic_MLP_A
...
class FusionModule(nn.Module):
    def __init__(self, inplanes=64, planes=9, hidden=64, num_layers=2,
                 mlp_type='c', kernel_size=3):
        super().__init__()
        self.inplanes = inplanes
        self.planes = planes
        self.hidden = hidden
